...
class ImagesGridActivity : BaseActivity() {
    ...
    override fun BuildContent() = ImagesGridPage() {
        navigateToDetailsPage(it.url)
    }

    private fun navigateToDetailsPage(url: String) {
        Intent(this, ImageDetailsActivity::class.java).apply {
...
fun ImagesGridPage(
    ...
    navigateToDetails: (NasaImage) -> Unit
) {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        when (val state = uiState.value) {
            ...
            is ImagesGridUiState.Success -> {
                ImageList(images = state.images, navigateToDetails)
            }
